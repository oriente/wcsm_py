...
class CalculateCentrality:
    ...
    def __init__(self, n_node, rt_list):
        self.node_num = n_node
        self.aggregated_list = rt_list
        self.graph_inv = []
    ...
    def calculate_degree(self):
        ...
        G.add_edges_from(self.aggregated_list)
        degree = G.degree()
    ...
    def calculate_between(self, sink):
        # calculate betweenness adptively
        #print self.graph_inv
        betweenness_attack = []
        G_inv = nx.Graph()
        G_inv.add_nodes_from(range(self.node_num))
        G_inv.add_edges_from(self.aggregated_list)
        betweenness = nx.betweenness_centrality(G_inv)
        for n in sink:
            betweenness[n] = -1
    ...
    def calculate_close(self, sink):
        ...
        G_inv.add_nodes_from(range(self.node_num))
        G_inv.add_edges_from(self.aggregated_list)
        closeness = nx.closeness_centrality(G_inv)
        print closeness
        for n in sink:
            closeness[n] = -1
