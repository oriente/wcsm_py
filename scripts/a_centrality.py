"""
asymmetric betweenness centrality measures.
"""
import heapq
...
__all__ = ['a_closenness_centrality',
           'a_betweenness_centrality',
           'betweenness_centrality',
           'edge_betweenness_centrality',
           'edge_betweenness']

def a_closeness_centrality(G, Sink=None, v=None, distance=None, normalized=True):
    ...
    # only calcualte the closeness for source node
    for node in Sink:
        nodes.remove(node)
    for n in nodes:
        sp=path_length(G,n)
        ...
        if totsp > 0.0 and len(G) > 1:
            closeness_centrality[n]= len(Sink) / totsp
...
def a_betweenness_centrality(G, k=None, normalized=True, weight=None, 
                           endpoints=False, 
                           seed=None, Sink=None):
    ...
    betweenness=_rescale(betweenness, len(G),
                         ...
                         k=k)
    # set betweenness as 0 for sink nodes
    for node in Sink:
